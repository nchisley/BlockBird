document.addEventListener('DOMContentLoaded', function() {
    ...
    const walletSearchInput = document.getElementById('wallet-search');  // New search input
  
    let allWallets = [];  // Store all wallets for filtering
    ...
    if (!window.walletListenersAdded) {
        ...
        walletSearchInput.addEventListener('input', handleWalletSearch);  // Add search event listener
    ...
    function loadWallets() {
        chrome.storage.local.get(['wallets'], function(result) {
            ...
            renderWalletList(allWallets);  // Render full list on load
        ...
        if (label && address) {
            chrome.storage.local.get(['wallets'], function(result) {
                ...
                if (addressExists) {
                    ...
                    saveWallets(wallets, () => {
                        allWallets = wallets;  // Update the full list
                        renderWalletList(allWallets);  // Re-render the wallet list
    ...
    // New function to handle the wallet search
    function handleWalletSearch() {
        ...
        renderWalletList(filteredWallets);  // Re-render the list with the filtered wallets
    }
